...
class RAGBot:
    def __init__(self, token, credentials):
        ...
        self.vector_store = FAISS.load_local(folder_path=os.path.abspath('data/db/'), embeddings=get_embeddings(), allow_dangerous_deserialization=True)
    ...
    def handle_answer_question(self, message):
        if message.text == 'ℹ️ Информация о боте':
            self.send_message(message.from_user.id, f'Я RAG-бот. Я могу ответить на любой вопрос, связанный с мировыми трендами в области искусственного интеллекта. Если хочешь загрузить свой документ, нажми кнопку "Добавить документ"')
